(() => {
    ...
    function changeAttributeValues(array, attributeName, value) {
        for (let i = 0; i < array.length; i++) array[i].setAttribute(attributeName, value);
    }
    function buildCircles(slides, isMobile = false) {
        if (isMobile) slides.forEach(((slide, i) => {
            const slideContent = slide.querySelector(".slide-solutions__content");
            slide.classList.remove("active");
            slide.classList.remove("not-active");
            slide.style = "";
            slide.removeAttribute("data-rotate");
            slideContent.style = "";
        })); else slides.forEach(((slide, i) => {
            const slideContent = slide.querySelector(".slide-solutions__content");
            const rotateValue = (360 / slides.length * i).toFixed(3);
            slide.style = `transform: rotate(${rotateValue}deg);`;
            slide.setAttribute("data-rotate", rotateValue);
            slideContent.style = `transform: rotate(${-rotateValue}deg);`;
        }));
    }
    function setPositionLines(progressLines, lastColumn) {
        let lastColumnHeight = lastColumn.clientHeight;
        if (window.innerWidth < 768) progressLines.style.bottom = `${(lastColumnHeight - 23) / 16}rem`; else progressLines.style.bottom = `${lastColumnHeight / 2 / 16}rem`;
    }
    function animationLines(animElements) {
        let myReq;
        const animData = {
            elements: animElements,
            activeIndex: 0,
            coordsTop: getCoords(animElements),
            scrollTop: calcScrollTop()
        };
        if (document.querySelector(".column-progress__icon")) {
            const progressCircles = document.querySelectorAll(".column-progress__icon");
            animData.circleSize = document.querySelector(".column-progress__icon").clientHeight;
            animData.lineElement = document.querySelector(".progress__line:last-child");
            animData.coordsCircles = getCoords(progressCircles, .5);
            animData.lineSize = animData.coordsCircles[animData.coordsCircles.length - 1] - animData.coordsCircles[0];
            animData.positionCircles = calcPositionCircles(animData);
        }
        const {elements, coordsTop, scrollTop} = animData;
        if (scrollTop > coordsTop[0]) elements.forEach(((elem, index) => {
            if (scrollTop > coordsTop[index]) animData.activeIndex = getActiveIndex(animData);
        }));
        window.addEventListener("scroll", (() => {
            if (animData.scrollTop - 50 < coordsTop[coordsTop.length - 1]) {
                animData.scrollTop = calcScrollTop();
                if (animData.scrollTop > coordsTop[animData.activeIndex]) myReq = requestAnimationFrame((() => {
                    animData.activeIndex = getActiveIndex(animData);
                    animData.lineElement.style.transform = `scaleY(${animData.positionCircles[animData.activeIndex - 1]}%)`;
                }));
            } else cancelAnimationFrame(myReq);
        }));
    }
    function calcScrollTop() {
        return window.scrollY + window.innerHeight * .7;
    }
    function getCoords(elements, addValue = 0) {
        let coords = [];
        elements.forEach((column => {
            coords.push(Math.floor(column.getBoundingClientRect().top + column.getBoundingClientRect().height * addValue));
        }));
        return coords;
    }
    function calcPositionCircles(animData) {
        const {coordsCircles, lineSize} = animData;
        let positions = [];
        coordsCircles.forEach((coord => {
            const position = ((coord - coordsCircles[0]) / lineSize * 100).toFixed(2);
            positions.push(Number(position));
        }));
        return positions;
    }
    function getActiveIndex(animData) {
        const {activeIndex, elements} = animData;
        if (!elements[activeIndex].classList.contains("show")) {
            elements[activeIndex].classList.add("show");
    ...
    function handleForm() {
        ...
        function validateForm(form) {
            ...
            for (let index = 0; index < inputsRequired.length; index++) {
                ...
                removeError(input);
                if (input.getAttribute("type") === "email" && testEmail(input)) addError(input); else if (input.getAttribute("type") === "tel" && input.value.trim().length !== 18) addError(input); else if (input.getAttribute("type") === "number" && (input.value.trim() === "" || input.value.trim() === "0")) addError(input); else if ((input.getAttribute("type") === "text" || input.tagName === "TEXTAREA") && input.value.trim().length < 2) addError(input); else if (input.getAttribute("type") === "checkbox" && input.checked === false) addError(input); else if (input.hasAttribute("data-date") && testDate(input)) addError(input);
            }
        }
    ...
    function onClickMenu(targetElement) {
        if (document.body.classList.contains("menu-open") && !targetElement.closest("#menu.open") && !targetElement.closest("[aria-controls='menu']")) {
            !targetElement.classList.contains("search-button") ? toggleLockBody(true, fixedElements, delay, bodyScrollBar) : null;
    ...
    function onClickSubmenu(targetElement) {
        if (targetElement.getAttribute("aria-expanded") === "false") {
            if (targetElement.parentElement.classList.contains("menu-header__item")) changeAttributeValues(document.querySelectorAll("[aria-expanded]"), "aria-expanded", "false"); else {
                const submenuList = targetElement.closest(".submenu__list");
                changeAttributeValues(submenuList.querySelectorAll("[aria-expanded]"), "aria-expanded", "false");
            }
            targetElement.setAttribute("aria-expanded", "true");
        } else {
            changeAttributeValues(targetElement.parentElement.querySelectorAll("[aria-expanded]"), "aria-expanded", "false");
    ...
    function subMenu() {
        ...
        submenuItems.forEach(((submenu, index) => {
            let submenuID;
            if (submenu.getAttribute("id")) submenuID = submenu.getAttribute("id"); else {
                submenu.setAttribute("id", `submenu-${index}`);
                submenuID = `submenu-${index}`;
            }
            if (submenu.previousElementSibling.tagName === "BUTTON") {
                submenu.previousElementSibling.setAttribute("aria-expanded", "false");
                submenu.previousElementSibling.setAttribute("aria-controls", submenuID);
            } else {
                const menuParentButton = submenu.closest("nav").querySelector(`[data-menu-parent="${submenuID}"] > button`);
                menuParentButton.setAttribute("aria-expanded", "false");
                menuParentButton.setAttribute("aria-controls", submenuID);
            }
        }));
    }
    function onClickOne(targetElement) {
        if (isMobile.any() && targetElement.parentElement.hasAttribute("data-menu-parent")) onClickSubmenu(targetElement);
        if (!targetElement.closest("[data-menu-parent]") && document.querySelectorAll("[data-menu-parent] > [aria-expanded='true']").length > 0) changeAttributeValues(document.querySelectorAll("[data-menu-parent] button"), "aria-expanded", "false");
    }
    function onKeyDownOne() {
        if (!isMobile.any() && (e.key === " " || e.key === "Enter") && e.target.parentElement.hasAttribute("data-menu-parent")) onClickSubmenu(e);
    }
    ...
    function lockBody(fixedElements, delay, scroller) {
        ...
        if (!lockStatus) {
            ...
            if (fixedElements) fixedElements.forEach((fixedElement => {
                fixedElement.style.paddingRight = scrollWidth;
            }));
            body.style.paddingRight = scrollWidth;
            body.classList.add("lock");
            lockStatus = true;
            !document.querySelector("._touch") && scroller ? toggleLockScrollBar() : null;
            setTimeout((function() {
                lockStatus = false;
            }), delay);
        }
    ...
    function toggleLockFromResize(menu, iconMenu, fixedElements, delay) {
        const lockBreakpoint = `(max-width: ${1023.98 / 16}em)`;
        let lockMatchMedia = window.matchMedia(lockBreakpoint);
        lockMatchMedia.addListener((function() {
            if (!lockMatchMedia.matches) {
                if (document.body.classList.contains("lock")) toggleLockBody(true, fixedElements, delay, bodyScrollBar);
                if (document.body.classList.contains("menu-open")) closeMenu(menu, iconMenu);
            }
        }));
    }
    ...
    class Popup {
        ...
        initPopup() {
            ...
            popupArray.forEach((popup => {
                const popupLabel = popup.querySelector("[data-popup-label]").innerHTML ? popup.querySelector("[data-popup-label]").innerHTML : "модальное окно";
                const popupCloseButtons = popup.querySelectorAll("[data-popup-close]");
                if (popupCloseButtons) popupCloseButtons.forEach((popupCloseButton => {
        ...
        getOpenPopup(targetElement) {
            ...
            if (targetElement.closest("[data-form]")) {
                ...
                formValidate(form);
                if (!form.querySelector(".input-error")) this.popupOpen ? this.openPopup() : null;
            } else this.popupOpen ? this.openPopup() : null;
        }
        openPopup() {
            if (!popup_lockStatus) {
        ...
        popupEvents() {
            this.popupOpen.addEventListener("click", function(e) {
                if (e.target.hasAttribute("data-popup-close") || !e.target.closest("[data-popup] [class*='__content']")) {
                    e.preventDefault();
                    this.nextOpen = false;
                    this.isOpen && !popup_lockStatus ? this.closePopup() : null;
    ...
    let _slideToggle = (target, duration = 500) => {
        if (!target.classList.contains("_slide")) {
            ...
            if (window.getComputedStyle(target).display === "none") return _slideDown(target, duration); else return _slideUp(target, duration);
        }
    };
    class Spollers {
        ...
        initSpollerBody() {
            ...
            if (this.spollerTitles) this.spollerTitles.forEach(((spollerTitle, index) => {
        ...
        onActionSpoller(e) {
            if (e.target.hasAttribute("data-spoller") || e.target.closest("[data-spoller]")) {
                ...
                if (!this.spollersItem.querySelectorAll("._slide").length) {
                    this.spollersOne && this.activeTitle.getAttribute("aria-expanded") === "false" ? this.hideSpollersBody() : null;
    ...
    function initSpollers() {
        if (document.querySelector("#spollers-menu")) {
            const spollersInfo = document.querySelector("#spollers-menu");
            new Spollers(spollersInfo, {
                mediaValue: "1023.98",
                typeMedia: "max",
                speed: 350
            });
    ...
    class Tabs {
        constructor(props, options) {
            ...
            if (props instanceof NodeList) props.forEach(((tabsItem, index) => {
        ...
        initTabs(tabsItem) {
            ...
            this.panels.forEach(((panel, index) => {
                ...
                if (this.options.animate) index === this.activeIndex ? this.panel.style.display = "block" : this.panel.style.display = "none";
                if (this.titles[index]) {
                    ...
                    this.title.addEventListener("click", function(e) {
                        ...
                        if (e.currentTarget.getAttribute("aria-selected") === "false") if (!this.options.animate) this.handlerTabsAction(); else !this.tabsItem.classList.contains("tab-change") ? this.handlerTabsAction() : null;
                    }.bind(this));
        ...
        handlerTabsAction() {
            ...
            if (!this.activeTitle.getAttribute("tab-index")) {
                ...
                !this.tabsItem.classList.contains("tab-spoller") ? this.panels[this.titles.indexOf(this.currentTitle)].setAttribute("hidden", "") : this.currentTitle.nextElementSibling.setAttribute("hidden", "");
                this.activeTitle.setAttribute("aria-selected", "true");
                this.activeTitle.setAttribute("tabindex", "0");
                this.activeTitle.focus();
                !this.tabsItem.classList.contains("tab-spoller") ? this.panels[this.titles.indexOf(this.activeTitle)].removeAttribute("hidden") : this.activeTitle.nextElementSibling.removeAttribute("hidden");
                this.currentTitle = this.activeTitle;
                this.switchTabs();
            }
        }
        switchTabs() {
            if (this.panels.length > 0) this.panels.forEach(((panel, index) => {
                if (this.titles[index]) if (this.titles[index].getAttribute("aria-selected") === "true") {
                    if (this.options.animate) !this.tabsItem.classList.contains("tab-spoller") ? _slideDown(panel, this.options.speed) : _slideDown(this.titles[index].nextElementSibling, this.options.speed);
                    location.hash = `tab-${this.itemIndex}-${index}`;
                } else if (this.options.animate) !this.tabsItem.classList.contains("tab-spoller") ? _slideUp(panel, this.options.speed) : _slideUp(this.titles[index].nextElementSibling, this.options.speed);
            }));
        }
        onKeyDown(e) {
        ...
        moveLeft() {
            !this.currentTab.previousElementSibling ? this.activeTitle = this.titles[this.titles.length - 1] : this.activeTitle = this.currentTab.previousElementSibling;
            this.handlerTabsAction();
        }
        moveRight() {
            !this.currentTab.nextElementSibling ? this.activeTitle = this.titles[0] : this.activeTitle = this.currentTab.nextElementSibling;
            this.handlerTabsAction();
        }
        setHash() {
    ...
    function initTabs() {
        if (document.querySelector(".tabs-services")) {
            const tabsServices = document.querySelector(".tabs-services");
            new Tabs(tabsServices, {});
        }
    }
    function goTo() {
        const menuLinks = document.querySelectorAll("[data-goto]");
        menuLinks.forEach((menuLink => {
            menuLink.addEventListener("click", goToBlock);
        }));
        function goToBlock(e) {
            e.preventDefault();
            const targetBlock = e.target.getAttribute("href");
            const targetBlockElement = document.querySelector(targetBlock);
            if (targetBlockElement) window.scrollTo({
                top: targetBlockElement.getBoundingClientRect().top + window.scrollY - 20,
                behavior: "smooth"
            }); else console.log(`[gotoBlock]: блока ${targetBlock} не существует на странице`);
        }
    }
    ...
    function swiper_buildSliders() {
        ...
        initSliders();
        if (document.querySelector(".slider-solutions")) initSliderSolutions();
    }
    swiper_buildSliders();
    function initSliderSolutions(isMobile = false) {
        if (isMobile || window.innerWidth < 768) sliderSolutions = new Swiper(".slider-solutions", {
            observer: true,
            observeParents: true,
            watchOverflow: true,
            slidesPerView: "auto",
            ...
            navigation: {
                nextEl: ".solutions [data-arrow-next]",
                prevEl: ".solutions [data-arrow-prev]"
            },
            breakpoints: {
                768: {
                    spaceBetween: 0
        ...
        }); else if (document.querySelector(".slider-solutions.swiper-initialized")) sliderSolutions.destroy();
    }
    function initSliders() {}
    document.querySelector(".header");
    const menu = document.querySelector("#menu");
    const iconMenu = document.querySelector("[aria-controls='menu']");
    ...
    if (detectBrowser() === "Firefox") document.querySelector("body").classList.add("_firefox");
    document.querySelector("input") ? focusInput() : null;
    document.querySelector("[data-form]") ? handleForm() : null;
    document.querySelector("#menu") ? toggleLockFromResize(menu, iconMenu, fixedElements, delay) : null;
    document.querySelector("[data-da]") ? initDynamicAdapt() : null;
    document.querySelector("[aria-controls='menu']") ? initMenu() : null;
    ...
    window.addEventListener("DOMContentLoaded", (function() {
        ...
        if (document.querySelector(".slider-solutions")) {
            ...
            const solutionsSlides = document.querySelectorAll(".slide-solutions");
            if (window.innerWidth > 767.98) buildCircles(solutionsSlides, false);
            matchMedia.addEventListener("change", (() => {
                if (matchMedia.matches) {
                    initSliderSolutions(true);
                    buildCircles(solutionsSlides, true);
                } else {
                    initSliderSolutions(false);
                    buildCircles(solutionsSlides, false);
                }
            }));
            solutionsSlides.forEach((slide => {
                slide.addEventListener("mouseover", (function(e) {
                    for (let i = 0; i < solutionsSlides.length; i++) {
                        const slideElem = solutionsSlides[i];
                        slideElem.classList.remove("hover");
                    }
                    if (!slide.classList.contains("hover")) slide.classList.add("hover");
                }));
            }));
            solutionsSlides.forEach((slide => {
                slide.addEventListener("click", (function(e) {
                    if (!slide.classList.contains("active") && window.innerWidth > 767.98) {
                        e.preventDefault();
                        const activeSlide = document.querySelector(".slide-solutions.active");
                        const slideContent = slide.querySelector(".slide-solutions__content");
                        const rotateValue = slide.getAttribute("data-rotate") ? Number(slide.getAttribute("data-rotate")) : 0;
                        if (activeSlide) {
                            const activeSlideContent = activeSlide.querySelector(".slide-solutions__content");
                            const rotateValueActiveSlide = activeSlide.getAttribute("data-rotate") ? activeSlide.getAttribute("data-rotate") : 0;
                            activeSlide.classList.remove("active");
                            activeSlide.classList.add("not-active");
                            activeSlide.style = `transform: rotate(${rotateValueActiveSlide}deg);`;
                            activeSlideContent.style = `transform: rotate(${-rotateValueActiveSlide}deg);`;
                        }
                        slide.classList.remove("not-active");
                        slide.classList.add("active");
                        slide.style = `transform: rotate(${rotateValue - 13}deg)`;
                        slideContent.style = `transform: rotate(${-rotateValue + 13}deg)`;
                        const notActiveSlides = document.querySelectorAll(".slide-solutions:not(.active)");
        ...
        if (document.querySelector(".progress__lines")) {
            const progressLines = document.querySelector(".progress__lines");
            const progressColumns = document.querySelectorAll(".column-progress__body");
            if (progressLines && progressColumns.length > 0) {
                const lastColumn = progressColumns[progressColumns.length - 1];
                window.addEventListener("resize", (function(e) {
                    setPositionLines(progressLines, lastColumn);
                }));
                setPositionLines(progressLines, lastColumn);
                animationLines(progressColumns);
            }
        }
    }));
    document.addEventListener("keydown", (function(e) {
